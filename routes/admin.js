...
//#region Validata Account
router.get("/valid-username", isAdmin, function(req, res, next) {
  accountModel.getAccountByUsername(req.query.username).then(rows => {
    if (rows.length > 0) {
...
router.get("/valid-email", isAdmin, function(req, res, next) {
  accountModel.getAccountByEmail(req.query.email).then(rows => {
    if (rows.length > 0) {
      return res.json(false);
    }
    return res.json(true);
...
router.get("/", isAdmin, async function(req, res, next) {
  try {
    const listTeacher = await adminModel.getAll(
      "accounts",
      "account_role",
      "teacher"
    );
...
router.get("/course", isAdmin, async function(req, res, next) {
  if (req.query.id) {
    try {
      const courseDetail = await adminModel.executeQuery(
        `select * from courses, co_ac where course_id = ${req.query.id} and course_id = co_ac_course`
      );
      const listSubject = await adminModel.findAll("subjects");
      const listTeacher = await adminModel.executeQuery(
        `select * from accounts where account_role = 'teacher'`
      );

      courseDetail[0].course_start_date = moment(
        courseDetail.course_start_date
      ).format("YYYY-MM-DD");
      courseDetail[0].course_end_date = moment(
        courseDetail.course_end_date
      ).format("YYYY-MM-DD");
    ...
    try {
      const listCourse = await adminModel.executeQuery(
        ` select * from courses join accounts join co_ac join subjects where course_subject = subject_id and course_id = co_ac_course and co_ac_account = account_id and co_ac_role = 'teacher'`
      );
      const listSubject = await adminModel.findAll("subjects");
      const listTeacher = await adminModel.executeQuery(
        `select * from accounts where account_role = 'teacher'`
...
router.get("/lesson", isAdmin, async function(req, res, next) {
  if (req.query.id) {
    try {
      const lessonDetail = await adminModel.getOne(
        "lessons",
        "lesson_id",
        req.query.id
      );
      const listCourse = await adminModel.findAll("courses");
      const listResource = await adminModel.getAll(
        "resources",
        "resource_lesson",
        lessonDetail.lesson_id
      );
    ...
    try {
      const listLesson = await adminModel.executeQuery(
        "select * from lessons, courses where lesson_course = course_id"
      );
      const listCourse = await adminModel.findAll("courses");
